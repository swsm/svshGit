...
import com.swsm.system.system.service.IDictIdxService;
...
public class DictIndexAction extends EntityHandler<DictIdx> {

    /**
     * 日志
     */
    private static Logger logger = LoggerFactory.getLogger(DictIndexAction.class);
    ...
    @Qualifier("dictIdxService")
    @Override
    public void setEntityServiceImpl(IEntityService<DictIdx> entityServiceImpl) {
        this.entityServiceImpl = entityServiceImpl;
        this.clazz = DictIdx.class;
    }
     ...
     * @throws Exception 异常
     */
    @RequestMapping(value = "/getDictIndex.mvc")
    @ResponseBody
    public void getDictIndexs(HttpServletRequest request, HttpServletResponse response) throws Exception {
        Filter filter;
        filter = createFilter(request);
        List<DictIdx> list;
        list = this.dictIdxService.getDictIdxForGrid(filter, DictIdx.class);
    ...
    private Filter createFilter(HttpServletRequest request) {
        ...
        if (request.getParameter("parentKey") != null && !request.getParameter("parentKey").isEmpty()) {
            query = new Query(Condition.EQ, request.getParameter("parentKey"));
     ...
     * @throws Exception 异常
     */
    @RequestMapping(value = "/saveOrUpdateDictIndex.mvc")
    @ResponseBody
    public void saveOrUpdateDictIndex(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String rows;
        rows = WebUtil.fetch(request, "storeDate");
        List<DictIdx> objList;
        objList = JSON.parseArray(rows, DictIdx.class);
        Jdf3UserSession userSession;
        userSession = super.getJdf3UserSession(request);
        MesBaseUtil.writeJsonStr(response, this.toJsonStrActionMessage(true,
                        this.dictIdxService.saveOrUpdateDictIndex(objList, userSession.getUserName())));
    }
    ...
    @RequestMapping(value = "/saveOrUpdateDictIndexDemo.mvc")
    @ResponseBody
    public void saveOrUpdateDictIndexDemo(HttpServletRequest request, HttpServletResponse response) throws Exception {
        DictIdx dictIndex;
        String  oldDictKey = null;
        if (request.getParameter("id") != null && !request.getParameter("id").isEmpty()) {
            //更新
            dictIndex = ((IEntityService<DictIdx>) this.dictIdxService)
                    .findEntity(DictIdx.class, request.getParameter("id"));
            oldDictKey = dictIndex.getDictKey();
        } else {
            dictIndex = new DictIdx();
        }
        String newDictKey;
        newDictKey = request.getParameter("dictKey");
        dictIndex.setDictName(request.getParameter("dictName"));
        dictIndex.setRemark(request.getParameter("remark"));
        Jdf3UserSession userSession;
        userSession = super.getJdf3UserSession(request);
        String result;
        result = this.dictIdxService.saveOrUpdateDictIndexDemo(dictIndex, oldDictKey, newDictKey,
                userSession.getUserName());
        //增加操作日志
        if ("success".equals(result)) {
            OperateContent operateContent;
            operateContent = new OperateContent();
            operateContent.setOperateName(dictIndex.getDictName());
            if (StringUtils.isEmpty(dictIndex.getUpdateUser())) {
     ...
     * @throws Exception 异常
     */
    @RequestMapping(value = "/deleteDictIndex.mvc")
    @ResponseBody
    public void deleteDictIndex(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String ids;
        ids = request.getParameter("ids");
        Jdf3UserSession userSession;
        userSession = super.getJdf3UserSession(request);
        String result;
        result = this.dictIdxService.deleteDictIndex(ids, userSession.getUserName());
        //增加操作日志
        if ("success".equals(result)) {
            String dictNames;
            dictNames = request.getParameter("dictNames");
            List<OperateContent> list;
            list = new ArrayList<OperateContent>();
            for (String dictName : dictNames.split(",")) {
                OperateContent operateContent;
                operateContent = new OperateContent();
                operateContent.setOperateName(dictName);
     ...
     * @throws Exception 异常
     */
    @RequestMapping(value = "/checkDictIndexUnique.mvc")
    @ResponseBody
    public void checkDictIndexUnique(HttpServletRequest request, HttpServletResponse response) throws Exception {
