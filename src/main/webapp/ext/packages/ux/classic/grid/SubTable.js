...
Ext.define('Ext.ux.grid.SubTable', {
    ...
    rowBodyTpl: ['<table class="' + Ext.baseCSSPrefix + 'grid-subtable" style="font-size:12px;"><tbody>',
        '{%',
    ...
    renderTable: function(out, rowValues) {
        var me = this,
            ...
            rec, column, i, j, value, val;

        out.push('<thead>');
        for (j = 0; j < numColumns; j++) {
            out.push('<th class="' + Ext.baseCSSPrefix + 'grid-subtable-header" style="text-align:left">', columns[j].text, '</th>');
        }
        out.push('</thead>');
        for (i = 0; i < recCount; i++) {
            ...
            for (j = 0; j < numColumns; j++) {
                ...
                if(column.qtip){
                	out.push('><div class="' + Ext.baseCSSPrefix + 'grid-cell-inner" style="width:' + column.width + 'px" data-qtip="'+ val +'">', value, '</div></td>');
                }else{
                	out.push('><div class="' + Ext.baseCSSPrefix + 'grid-cell-inner" style="width:' + column.width + 'px" >', value, '</div></td>');
                }
            }
            out.push('</tr>');
        }
